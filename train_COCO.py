...
from dataset import COCOMultiSegDataset, COCOValMultiSegDataset
...
def train(
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=val_batch_size, shuffle=True, num_workers=1)
    ...
    while step < train_steps:
        ...
        prev_prompts = batch["ref_prompt"] # Attetion! Cannot use prev_prompt for shape mismatch! (b, 3, 77) Or transpose will be needed.
        prev_prompt_ids = []
        ...
        ref_img_features = []
        for i in range(3):
            noisy_ref_image = noise_scheduler.add_noise(ref_image_list[i], ref_noise, ref_timesteps)
            prev_encoder_hidden_states = text_encoder(t_prev_prompt_ids[i].to(accelerator.device))[0]
            img_dif_conditions = unet(noisy_ref_image, ref_timesteps, encoder_hidden_states=prev_encoder_hidden_states, return_dict=False)[1]
